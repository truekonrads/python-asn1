...
class Decoder(object):
    ...
    def peek_length(self):
        """This method returns the current ASN.1 tag's length (i.e. the tag that a
        subsequent `Decoder.read()` call would return) without updating the
        decoding offset. 
        ...
        if self.m_length is None:
            self.m_length = self._read_length()
